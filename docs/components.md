...
- Music keeps minimal mode quiet with a `72px` circular artwork and no spectrum.
  ...
  rounded accent bars. A controlled random envelope selects two or three radial
  peaks, keeps them at least roughly `76°` apart, selects a new pattern every
  `1.6–2.6s`, and morphs between patterns over `650ms`. Each frequency owns a
  slowly adapting floor and ceiling, allowing both bass and treble bars to
  travel from the quiet `2px` baseline to the moving directional envelope
  without pinning the strongest frequency on every frame. Bars have roughly
  `40px` of travel at the minimum panel size and fade in only after a valid
